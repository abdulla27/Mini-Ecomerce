...
module.exports = (app, express, mysqlcon) => {
    ...
    router.post("/buyer_registeration",async(req,res)=>{
        ...
        let a= await mysqlcon(`insert into buyer_registeration (username,password,email,gender,phone,buyer_id,role) values ("${stusername}","${stpassword}","${stemail}","${stgender}","${stphone}","${stbuyer_id}",2)`)
