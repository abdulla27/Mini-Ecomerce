module.exports = (app, express, mysqlcon) => {
  ...
  router.get("/home", acces, async (req, res) => {
    console.log(req.body.mydata);
    let value = await mysqlcon(`select*from product`);
    // let value = await prisma.product.findMany();
    res.render("home", { data: value });
  });

  router.get("/public_home", acces2, async (req, res) => {
    console.log(req.body.mydata);
    let value = await mysqlcon(`select*from product`);
    // let value = await prisma.product.findMany();
  ...
  router.get("/about", acces, (req, res) => {
    res.render("about", {
      data: "this is your about information please don't share it anyone ha ha ha ha ha.....",
    });
  });
